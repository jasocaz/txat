...
const ALLOWED_TRANSCRIBE_MODELS = ['gpt-4o-mini-transcribe', 'gpt-4o-transcribe'];
...
function buildSessionConfig(transcribeModel: string) {
...
export async function POST(req: NextRequest) {
  ...
  const requested = req.nextUrl.searchParams.get('transcribeModel');
  const transcribeModel =
    requested && ALLOWED_TRANSCRIBE_MODELS.includes(requested)
      ? requested
      : DEFAULT_TRANSCRIBE_MODEL;

  console.log('[realtime-session] model:', REALTIME_MODEL, 'transcribe:', transcribeModel);
  ...
  try {
    ...
    fd.set('sdp', sdp);
    fd.set('session', buildSessionConfig(transcribeModel));
